function Import-PokemonCollection {
    ...
    switch ($ext) {
        ...
        '.xlsx' {
            if (-not (Get-Module -ListAvailable -Name ImportExcel)) {
                throw 'Importing .xlsx requires the ImportExcel PowerShell module. Install-Module ImportExcel -Scope CurrentUser, or export the sheet as CSV.'
            }
    ...
    $targetPath = if ($DestinationPath) { $DestinationPath } else { (Get-PokemonSearchSettings).CollectionPath }
    if ($PSCmdlet.ShouldProcess($targetPath, "Import $($records.Count) Pokemon card records")) {
...
function Add-PokemonCollectionCard {
    ...
    param(
        ...
        [string]$Grade,
        [ValidateRange(1,9999)][int]$Quantity = 1,
    ...
    $record = [pscustomobject]@{
        ...
        Variant=$Variant; Condition=$Condition; Grade=$Grade; Quantity=$Quantity
        PurchasePrice=if ($PSBoundParameters.ContainsKey('PurchasePrice')) { $PurchasePrice } else { $null }
        Notes=$Notes; Source='Add-PokemonCollectionCard'; AddedAt=(Get-Date).ToString('o')
    }
...
function Get-PokemonCollectionValue {
    ...
    foreach ($row in $collection) {
        ...
        $market = if ($price -and $null -ne $price.Market) { [double]$price.Market } else { 0 }
    ...
    $quantityTotal = ($collection | Measure-Object Quantity -Sum).Sum
    if ($null -eq $quantityTotal) { $quantityTotal = 0 }
    if ($null -eq $marketTotal) { $marketTotal = 0 }
    $summary = [pscustomobject]@{
        Records=$collection.Count
        Quantity=$quantityTotal
        PricedRecords=$items.Count
        MarketValue=[math]::Round($marketTotal,2)
        KnownCostBasis=if ($null -ne $costTotal) {[math]::Round($costTotal,2)}else{$null}
        UnrealizedDifference=if ($null -ne $costTotal) {[math]::Round(($marketTotal-$costTotal),2)}else{$null}
    }
...
function Get-PokemonSetChecklist {
    ...
    foreach ($rawCard in @($response.data)) {
        ...
        $matches = @($owned | Where-Object {
            ($_.CardId -and $_.CardId -eq $card.Id) -or
            ($_.SetId -eq $card.SetId -and [string]$_.Number -eq [string]$card.Number)
        })
        $qty = ($matches | Measure-Object Quantity -Sum).Sum
        ...
        [pscustomobject]@{
            Number=$card.Number; Name=$card.Name; Rarity=$card.Rarity; Owned=($qty -gt 0); Quantity=$qty
            Market=if ($preferred) {$preferred.Market}else{$null}; Variant=if ($preferred) {$preferred.Variant}else{$null}
            CardId=$card.Id; Image=$card.ImageSmall
        }
    }
}
